            ...
            if (select(sock + 1, &readfds, &writefds, NULL, &tv) > 0) {
                ...
                if (is_sending && FD_ISSET(sock, &writefds)) {
                    ...
                    if (n > 0) {
                        ...
                        // 1. Create a "File finished" message
                        char completion_msg[256];
                        snprintf(completion_msg, sizeof(completion_msg), "✅ Sent file: %s", current_sending_filename);

                        // 2. Send it as a regular chat packet so everyone sees it once
                        PacketHeader h_done = {TYPE_CHAT, strlen(completion_msg)};
                        strncpy(h_done.sender_name, my_username, 49);
                        send(sock, &h_done, sizeof(h_done), 0);
                        send(sock, completion_msg, h_done.payload_size, 0);
